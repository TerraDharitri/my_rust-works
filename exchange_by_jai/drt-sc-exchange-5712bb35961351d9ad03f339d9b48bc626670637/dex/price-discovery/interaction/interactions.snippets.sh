...
#  issueRedeemToken DTKREWARedeemToken DTKREWART 18
...
deployPriceDiscoveryContract() {
    ...
    locking_address="0x$(drtpy wallet bech32 --decode $LOCKING_SC_ADDRESS)"
    ...
    ADDRESS=$(drtpy data parse --file="deploy-price-discovery.interaction.json" --expression="data['emitted_tx']['address']")
...
upgradePriceDiscoveryContract() {
    ...
    drtpy --verbose contract upgrade $PRICE_DISCOVERY_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-limit=350000000 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
        --bytecode="../output/price-discovery.wasm" \
...
setLockingScAddress() {
    locking_sc_address="0x$(drtpy wallet bech32 --decode $LOCKING_SC_ADDRESS)"

    drtpy --verbose contract call $PRICE_DISCOVERY_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-limit=100000000 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
