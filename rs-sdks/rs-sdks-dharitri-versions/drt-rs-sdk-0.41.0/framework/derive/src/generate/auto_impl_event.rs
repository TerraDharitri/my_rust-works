...
pub fn generate_legacy_event_impl(m: &Method, event_id_bytes: &[u8]) -> proc_macro2::TokenStream {
    ...
    let topic_conv_snippets: Vec<proc_macro2::TokenStream> = m
        ...
        .map(|arg| {
            let result = if topic_index < nr_args_no_self {
                ...
                let pat = &arg.pat;
                quote! {
                    let data_vec = match dharitri_sc::codec::top_encode_to_vec_u8(&#pat) {
                        Result::Ok(data_vec) => data_vec,
                        Result::Err(encode_err) => dharitri_sc::api::ErrorApiImpl::signal_error(
    ...
    quote! {
        #msig {
            ...
            #(#topic_conv_snippets)*
            dharitri_sc::api::LogApiImpl::write_legacy_log(
