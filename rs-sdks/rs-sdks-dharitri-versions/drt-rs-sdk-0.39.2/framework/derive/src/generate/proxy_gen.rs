...
pub fn generate_proxy_endpoint(m: &Method, endpoint_name: String) -> proc_macro2::TokenStream {
    ...
    let mut multi_expr = quote! { dharitri_sc::types::ManagedVec::<Self::Api, DcdtTokenPayment<Self::Api>>::new() };
    ...
    if token_count > 0 || nonce_count > 0 || payment_count > 0 {
        ...
        if token_count == 0 && nonce_count == 0 {
            contract_call_type = quote! { dharitri_sc::types::ContractCallWithRewa };
            contract_call_init = quote! {
                let mut ___contract_call___ = dharitri_sc::types::ContractCallWithRewa::new(
                    ___address___,
                    #endpoint_name,
                    #payment_expr,
                );
            ...
            contract_call_type = quote! { dharitri_sc::types::ContractCallWithRewaOrSingleDcdt };
            contract_call_init = quote! {
                let mut ___contract_call___ = dharitri_sc::types::ContractCallWithRewaOrSingleDcdt::new(
                    ___address___,
                    #endpoint_name,
                    #token_expr,
                    #nonce_expr,
        ...
        contract_call_type = quote! { dharitri_sc::types::ContractCallWithMultiDcdt };
        contract_call_init = quote! {
            let mut ___contract_call___ = dharitri_sc::types::ContractCallWithMultiDcdt::new(
                ___address___,
                #endpoint_name,
                #multi_expr,
            );
