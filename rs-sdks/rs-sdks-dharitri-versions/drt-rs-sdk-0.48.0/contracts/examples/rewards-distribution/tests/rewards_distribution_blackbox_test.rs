...
use dharitri_sc::{
    ...
    types::{
        Address, BigUint, RewaOrDcdtTokenIdentifier, ManagedVec, MultiValueEncoded,
...
use dharitri_sc_scenario::{
    ...
    scenario_model::{
        ...
        SetStateStep, TxDCDT, TypedResponse,
    },
...
const REWARDS_DISTRIBUTION_PATH_EXPR: &str = "drtsc:output/rewards-distribution.drtsc.json";
const SEED_NFT_MINTER_ADDRESS_EXPR: &str = "sc:seed-nft-minter";
const SEED_NFT_MINTER_PATH_EXPR: &str = "drtsc:../seed-nft-minter/output/seed-nft-minter.drtsc.json";
            ...
            ScDeployStep::new()
                ...
                .call(
                    self.seed_nft_minter_contract
                        .init(TokenIdentifier::from_dcdt_bytes(NFT_TOKEN_ID)),
                ),
        );
...
fn test_raffle_and_claim() {
    ...
    let nft_payments: Vec<TxDCDT> = nft_nonces
        .iter()
        .map(|nonce| TxDCDT {
            dcdt_token_identifier: NFT_TOKEN_ID.into(),
            nonce: (*nonce).into(),
            dcdt_value: 1u64.into(),
        })
        .collect();
    ...
    for nonce in nft_nonces.iter() {
        alice_account =
            alice_account.dcdt_nft_balance(NFT_TOKEN_ID_EXPR, *nonce, "1", Option::<&[u8]>::None);
    }
    ...
    state.world.sc_call(
        ScCallStep::new()
            .from(ALICE_ADDRESS_EXPR)
            .multi_dcdt_transfer(nft_payments.clone())
            .call(
                state
                    .rewards_distribution_contract
    ...
    state.world.sc_call(
        ScCallStep::new()
            .from(ALICE_ADDRESS_EXPR)
            .multi_dcdt_transfer(nft_payments)
            .call(
                state
                    .rewards_distribution_contract
                    .claim_rewards(0u64, 0u64, reward_tokens),
