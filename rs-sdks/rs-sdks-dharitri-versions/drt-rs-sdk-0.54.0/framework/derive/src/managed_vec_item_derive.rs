    ...
    match fields {
        syn::Fields::Named(fields_named) => fields_named
            ...
            .map(|field| {
                ...
                let type_name = &field.ty;
                quote! {
                    dharitri_sc::types::ManagedVecItem::into_byte_writer(self.#field_ident, |bytes| {
                        let next_index = index + <#type_name as dharitri_sc::types::ManagedVecItem>::payload_size();
...
fn generate_payload_buffer_snippet() -> proc_macro2::TokenStream {
    quote! {
        let mut payload = <Self::PAYLOAD as dharitri_sc::types::ManagedVecItemPayload>::new_buffer();
        let payload_slice = dharitri_sc::types::ManagedVecItemPayload::payload_slice_mut(&mut payload);
    }
}
