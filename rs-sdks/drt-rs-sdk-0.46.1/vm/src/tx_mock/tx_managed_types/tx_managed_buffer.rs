...
impl TxManagedTypes {
    ...
    pub fn mb_get_vec_of_dcdt_payments(&self, source_handle: RawHandle) -> Vec<TxTokenTransfer> {
        let mut result = Vec::new();
        let data = self.mb_get(source_handle);
        assert!(
            data.len() % 16 == 0,
...
pub mod tests {
    ...
    fn test_vec_of_dcdt_payments() {
        ...
        m_types.mb_set_vec_of_dcdt_payments(handle, transfers.as_slice());
        let retrieved = m_types.mb_get_vec_of_dcdt_payments(handle);
