...
#[numbat_wasm_derive::contract(DcdtNftMarketplaceImpl)]
pub trait DcdtNftMarketplace {
	...
	fn auction_token(
		...
		require!(
			self.call_value().dcdt_value() == BigUint::from(NFT_AMOUNT),
			"Token is not an NFT"
		);
		require!(
	...
	fn bid(&self, nft_type: TokenIdentifier, nft_nonce: u64) -> SCResult<()> {
		...
		if auction.current_winner != Address::zero() {
			...
			self.send().direct_dcdt_nft_via_transfer_exec(
				&auction.current_winner,
				&auction.payment_token.token_type.as_dcdt_identifier(),
	...
	fn end_auction(&self, nft_type: TokenIdentifier, nft_nonce: u64) -> SCResult<()> {
		...
		if auction.current_winner != Address::zero() {
			let nft_info = self.get_dcdt_token_data(
				&self.get_sc_address(),
			...
			let token = auction.payment_token.token_type.as_dcdt_identifier();
