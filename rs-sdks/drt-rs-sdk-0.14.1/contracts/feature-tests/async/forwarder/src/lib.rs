...
#[numbat_wasm_derive::contract(ForwarderImpl)]
pub trait Forwarder {
	#[init]
	fn init(&self) {}
	...
	fn send_dcdt_twice(
		...
		self.send()
			.direct_dcdt_via_transf_exec(to, token_id.as_slice(), amount_first_time, data);
		self.send()
			.direct_dcdt_via_transf_exec(to, token_id.as_slice(), amount_second_time, data);
	}
