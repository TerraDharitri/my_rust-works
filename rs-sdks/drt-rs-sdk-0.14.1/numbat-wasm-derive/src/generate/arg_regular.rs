	...
	match &arg.ty {
		syn::Type::Reference(type_reference) => {
			...
			if let syn::Type::Slice(slice_type) = &*type_reference.elem {
				...
				quote! {
					numbat_wasm::load_single_arg::<T, Box<[#slice_elem]>>(self.api.clone(), #arg_index_expr, #arg_name_expr)
				}
			} else {
				...
				if let syn::Type::Path(syn::TypePath { path, .. }) = referenced_type {
					if let Some(ident) = path.get_ident() {
						if *ident == "str" {
							...
							return quote! {
								numbat_wasm::load_single_arg::<T, Box<str>>(self.api.clone(), #arg_index_expr, #arg_name_expr)
							};
						}
					}
				}
				...
				quote! {
					numbat_wasm::load_single_arg::<T, #referenced_type>(self.api.clone(), #arg_index_expr, #arg_name_expr)
				}
			}
		},
		_ => {
			quote! {
				numbat_wasm::load_single_arg::<T, #arg_ty>(self.api.clone(), #arg_index_expr, #arg_name_expr)
			}
		},
	}
}
