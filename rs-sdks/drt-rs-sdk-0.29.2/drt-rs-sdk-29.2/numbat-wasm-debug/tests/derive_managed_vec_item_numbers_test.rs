...
fn struct_with_numbers_static() {
    assert_eq!(
        <StructWithNumbers as numbat_wasm::types::ManagedVecItem>::PAYLOAD_SIZE,
        16
    );
    assert!(<StructWithNumbers as numbat_wasm::types::ManagedVecItem>::SKIPS_RESERIALIZATION);
}
...
fn struct_to_bytes_writer() {
    ...
    let mut arr: [u8; 16] =
        [0u8; <StructWithNumbers as numbat_wasm::types::ManagedVecItem>::PAYLOAD_SIZE];

    <StructWithNumbers as numbat_wasm::types::ManagedVecItem>::to_byte_writer(&s, |bytes| {
        arr[0..<StructWithNumbers as numbat_wasm::types::ManagedVecItem>::PAYLOAD_SIZE]
...
fn struct_from_bytes_reader() {
    ...
    let struct_from_bytes =
        <StructWithNumbers as numbat_wasm::types::ManagedVecItem>::from_byte_reader(|bytes| {
