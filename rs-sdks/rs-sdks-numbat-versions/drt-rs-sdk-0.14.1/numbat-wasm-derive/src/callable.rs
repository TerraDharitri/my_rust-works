...
pub fn process_callable(
	...
	let main_definition = quote! {
			...
			SA: SendApi<BigUint> + Clone + 'static,
		{
			pub api: SA,
			pub address: Address,
			pub token: numbat_wasm::types::TokenIdentifier,
			pub payment: BigUint,
			_phantom1: core::marker::PhantomData<BigInt>,
			_phantom2: core::marker::PhantomData<BigUint>,
		}

		impl<SA, BigInt, BigUint> numbat_wasm::types::ContractProxy<SA, BigInt, BigUint> for #callable_impl_name<SA, BigInt, BigUint>
		where
			BigUint: BigUintApi + 'static,
			BigInt: BigIntApi<BigUint> + 'static,
			SA: SendApi<BigUint> + Clone + 'static,
		{
			fn new(api: SA, address: Address) -> Self {
				#callable_impl_name {
					api,
					address,
					token: numbat_wasm::types::TokenIdentifier::rewa(),
