...
pub fn contract_implementation(
	...
	let function_selector = quote! {
	  impl <T, BigInt, BigUint> numbat_wasm::api::CallableContract<T> for #contract_impl_ident<T, BigInt, BigUint>
	  #api_where
	  {
		...
		fn clone_contract(&self) -> Box<dyn numbat_wasm::api::CallableContract<T>> {
	  ...
	  impl <T, BigInt, BigUint> numbat_wasm::api::ContractWithAbi for #contract_impl_ident<T, BigInt, BigUint>
	  #api_where
	  {
