use crate::types::{
	Address, ArgBuffer, AsyncCall, BoxedBytes, TokenIdentifier, TransferRewaExecute,
	TransferDcdtExecute, TransferExecute,
};
use crate::{
	api::{BigUintApi, SendApi, DCDT_TRANSFER_STRING},
	...
	BigUint: BigUintApi,
{
	...
	fn convert_to_dcdt_transfer_call(self) -> Self {
		if !self.token.is_rewa() {
			...
			new_arg_buffer.push_argument_bytes(self.token.as_dcdt_identifier());
	...
	/// Produces an REWA (or no value) transfer-execute call, no callback.
	/// Will always result in a `transferValueExecute` call.
	pub fn transfer_rewa_execute(self) -> TransferRewaExecute<BigUint> {
		TransferRewaExecute {
			to: self.to,
			rewa_payment: self.payment,
	...
	/// Produces an DCDT transfer-execute call, no callback.
	/// Will always result in a `transferDCDTExecute` call.
	pub fn transfer_dcdt_execute(self) -> TransferDcdtExecute<BigUint> {
		TransferDcdtExecute {
			to: self.to,
	...
	/// Will result in either a `transferValueExecute` or a `transferDCDTExecute` call, depending on input.
