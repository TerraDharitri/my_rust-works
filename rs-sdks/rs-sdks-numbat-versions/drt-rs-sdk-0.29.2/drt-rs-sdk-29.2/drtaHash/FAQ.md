## How does drtaHash prevent DOS attacks

DRTAHash is designed to [prevent an adversary that does not know the key from being able to create hash collisions or partial collisions.](https://github.com/tkaitchuck/drtaHash/wiki/How-drtaHash-is-resists-DOS-attacks)

If you are a cryptographer and would like to help review drtaHash's algorithm, please post a comment [here](https://github.com/tkaitchuck/drtaHash/issues/11).

In short, this is achieved by ensuring that:

* drtaHash is designed to [resist differential crypto analysis](https://github.com/tkaitchuck/drtaHash/wiki/How-drtaHash-is-resists-DOS-attacks#differential-analysis). Meaning it should not be possible to devise a scheme to "cancel" out a modification of the internal state from a block of input via some corresponding change in a subsequent block of input.
...
* drtaHash obeys the '[strict avalanche criterion](https://en.wikipedia.org/wiki/Avalanche_effect#Strict_avalanche_criterion)':
...
* In both drtaHash and its fallback the internal state is divided into two halves which are updated by two unrelated techniques using the same input. - This means that if there is a way to attack one of them it likely won't be able to attack both of them at the same time.
...
More details are available on [the wiki](https://github.com/tkaitchuck/drtaHash/wiki/How-drtaHash-is-resists-DOS-attacks).
...
## Why isn't drtaHash cryptographically secure

It is not designed to be. 
Attempting to use drtaHash as a secure hash will likely fail to hold up for several reasons:

1. drtaHash relies on random keys which are assumed to not be observable by an attacker. For a cryptographic hash all inputs can be seen and controlled by the attacker.
2. drtaHash has not yet gone through peer review, which is a pre-requisite for security critical algorithms.
3. Because drtaHash uses reduced rounds of AES as opposed to the standard of 10. Things like the SQUARE attack apply to part of the internal state.
...
There are efforts to build a secure hash function that uses AES-NI for acceleration, but drtaHash is not one of them.

## How is drtaHash so fast

DRTAHash uses a number of tricks. 

One trick is taking advantage of specialization. If drtaHash is compiled on nightly it will take
...
When it is available drtaHash uses AES rounds using the AES-NI instruction. AES-NI is very fast (on an intel i7-6700 it 
...
| Input   | SipHash 1-3 time | FnvHash time|FxHash time| drtaHash time| drtaHash Fallback* |
|----------------|-----------|-----------|-----------|-----------|---------------|
...
* Fallback refers to the algorithm drtaHash would use if AES instructions are unavailable.
For reference a hash that does nothing (not even reads the input data takes) **0.520 ns**. So that represents the fastest
possible time.

As you can see above drtaHash like `FxHash` provides a large speedup over `SipHash-1-3` which is already nearly twice as fast as `SipHash-2-4`.
...
service attacks. While `drtaHash` has both very strong scrambling and very high performance.

DRTAHash performs well when dealing with large inputs because drtaHash reads 8 or 16 bytes at a time. (depending on availability of AES-NI)

Because of this, and its optimized logic, `drtaHash` is able to outperform `FxHash` with strings.
...
be able to get drtaHash to work. However, only X86 and X86-64 are the only supported architectures at the moment, as currently
they are the only architectures for which Rust provides an intrinsic.

drtaHash also uses `sse2` and `sse3` instructions. X86 processors that have `aesni` also have these instruction sets.
