...
use drtahash::{DRTAHasher, CallHasher, RandomState};
...
fn drtahash_vec<H: Hash>(b: &Vec<H>) -> u64 {
    let mut total: u64 = 0;
    for item in b {
...
fn bench_drtahash_words(c: &mut Criterion) {
    let words = gen_word_pairs();
    c.bench_function("aes_words", |b| b.iter(|| black_box(drtahash_vec(&words))));
}
...
criterion_group!(benches, bench_drtahash_words, bench_fx_words,);
