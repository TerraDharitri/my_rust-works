//! DRTAHash is a hashing algorithm is intended to be a high performance, (hardware specific), keyed hash function.
...
//! # How drtaHash works
//!
//! drtaHash uses the hardware AES instruction on x86 processors to provide a keyed hash function.
//! drtaHash is not a cryptographically secure hash.
//!
//! # Example
//! ```
...
//! map.insert(12, 34);
//! ```
//! For convinence wrappers called `DRTAHashMap` and `DRTAHashSet` are also provided.
//! These to the same thing with slightly less typing.
//! ```ignore
//! use drtahash::DRTAHashMap;
//!
//! let mut map: DRTAHashMap<i32, i32> = DRTAHashMap::with_capacity(4);
//! map.insert(12, 34);
//! map.insert(56, 78);
//! ```
...
pub use crate::aes_hash::DRTAHasher;

#[cfg(not(any(
    all(any(target_arch = "x86", target_arch = "x86_64"), target_feature = "aes", not(miri)),
    all(any(target_arch = "arm", target_arch = "aarch64"), target_feature = "crypto", not(miri), feature = "stdsimd")
)))]
pub use crate::fallback_hash::DRTAHasher;
pub use crate::random_state::RandomState;

pub use crate::specialize::CallHasher;

#[cfg(feature = "std")]
pub use crate::hash_map::DRTAHashMap;
#[cfg(feature = "std")]
pub use crate::hash_set::DRTAHashSet;
...
/// [DRTAHasher]s in order to hash the keys of the map.
///
...
/// use std::collections::HashMap;
///
/// let mut map: HashMap<i32, i32, BuildHasherDefault<DRTAHasher>> = HashMap::default();
/// map.insert(12, 34);
/// ```
///
...
impl Default for DRTAHasher {
    /// Constructs a new [DRTAHasher] with fixed keys.
    ...
    ///
    /// ```
    /// use drtahash::DRTAHasher;
    /// use std::hash::Hasher;
    ///
    /// let mut hasher_1 = DRTAHasher::default();
    /// let mut hasher_2 = DRTAHasher::default();
    ///
...
mod test {
    ...
    fn test_default_builder() {
        ...
        let mut map = HashMap::<u32, u64, BuildHasherDefault<DRTAHasher>>::default();
        map.insert(1, 3);
    }
    ...
    #[test]
    fn test_non_zero() {
        let mut hasher1 = DRTAHasher::new_with_keys(0, 0);
        let mut hasher2 = DRTAHasher::new_with_keys(0, 0);
    ...
    #[test]
    fn test_drtahasher_construction() {
        let _ = DRTAHasher::new_with_keys(1234, 5678);
    }
}
