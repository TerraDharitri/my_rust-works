...
///
/// Instances of [`DRTAHasher`] represent state that is updated while hashing data.
///
...
impl DRTAHasher {
    ...
    ///
    /// Normally hashers are created via `DRTAHasher::default()` for fixed keys or `RandomState::new()` for randomly
    ...
    ///
    /// let mut hasher = DRTAHasher::new_with_keys(1234, 5678);
    ///
...
#[cfg(feature = "specialize")]
pub(crate) struct DRTAHasherU64 {
...
#[cfg(feature = "specialize")]
pub(crate) struct DRTAHasherFixed(pub DRTAHasher);

/// A specialized hasher for fixed size primitives larger than 64 bits.
#[cfg(feature = "specialize")]
impl Hasher for DRTAHasherFixed {
    #[inline]
    fn finish(&self) -> u64 {
        self.0.short_finish()
    }
...
#[cfg(feature = "specialize")]
pub(crate) struct DRTAHasherStr(pub DRTAHasher);
...
mod tests {
    ...
    #[test]
    fn test_default() {
        let hasher_a = DRTAHasher::default();
        ...
        let hasher_b = DRTAHasher::default();
