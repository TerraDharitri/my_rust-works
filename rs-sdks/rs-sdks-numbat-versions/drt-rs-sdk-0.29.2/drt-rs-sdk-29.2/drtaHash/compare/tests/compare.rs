use drtahash::{CallHasher, RandomState};
use criterion::*;
...
fn drtahash<K: Hash>(k: &K, builder: &RandomState) -> u64 {
...
fn compare_drtahash(c: &mut Criterion) {
    let builder = RandomState::new();
    let test = "compare_drtahash";
