...
fn struct_with_numbers_static() {
    assert_eq!(
        <ManagedStructWithBigUint<DebugApi> as dharitri_sc::types::ManagedVecItem>::PAYLOAD_SIZE,
        8
    );
    assert!(
        !<ManagedStructWithBigUint<DebugApi> as dharitri_sc::types::ManagedVecItem>::SKIPS_RESERIALIZATION
    );
}
...
fn managed_struct_to_bytes_writer() {
    ...
    <ManagedStructWithBigUint<DebugApi> as dharitri_sc::types::ManagedVecItem>::to_byte_writer(
        &s,
        |bytes| {
            arr[0..<ManagedStructWithBigUint::<DebugApi> as dharitri_sc::types::ManagedVecItem>::PAYLOAD_SIZE].copy_from_slice(bytes);
...
fn managed_struct_from_bytes_reader() {
    ...
    let struct_from_bytes =
        <ManagedStructWithBigUint<DebugApi> as dharitri_sc::types::ManagedVecItem>::from_byte_reader(
            |bytes| {
