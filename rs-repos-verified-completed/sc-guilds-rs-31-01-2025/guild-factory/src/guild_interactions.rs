    ...
    + utils::UtilsModule
{
    ...
    fn request_rewards(&self, amount: BigUint, is_query: bool) -> BigUint {
        ...
        let reward_payment = DcdtTokenPayment::new(guild_config.farming_token_id, 0, total_request);
        if !is_query {
            self.send()
                .direct_non_zero_dcdt_payment(&caller, &reward_payment);
        }
    ...
    fn check_payment_is_farming_token(&self) -> DcdtTokenPayment {
