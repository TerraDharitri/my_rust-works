use transaction::{BatchId, OperationDcdtPayment, PaymentsVec, Transaction, TxNonce};
...
pub struct CheckMustRefundArgs<'a, M: ManagedTypeApi> {
    pub token: &'a OperationDcdtPayment<M>,
    pub roles: DcdtLocalRoleFlags,
    ...
    + max_bridged_amount_module::MaxBridgedAmountModule
{
    fn check_must_refund(&self, args: CheckMustRefundArgs<Self::Api>) -> bool {
        let token_balance = self.blockchain().get_dcdt_balance(
            args.sc_address,
        ...
        if token_balance < args.token.token_data.amount {
            if args.token.token_nonce == 0 {
                if !args.roles.has_role(&DcdtLocalRole::Mint) {
                    return true;
                }
    ...
    fn has_nft_roles(
        &self,
        payment: &OperationDcdtPayment<Self::Api>,
        roles: DcdtLocalRoleFlags,
    ) -> bool {
        if !roles.has_role(&DcdtLocalRole::NftCreate) {
            return false;
        }

        if payment.token_data.amount > NFT_AMOUNT && !roles.has_role(&DcdtLocalRole::NftAddQuantity)
        {
            return false;
        }
