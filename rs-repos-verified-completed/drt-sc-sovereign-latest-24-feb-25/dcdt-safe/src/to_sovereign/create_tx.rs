    ...
    + token_mapping::TokenMappingModule
{
        ...
        for payment in &payments {
            ...
            if is_sov_chain {
                ...
                let mvx_to_sov_token_id_mapper =
                    self.dharitri_to_sovereign_token_id_mapper(&payment.token_identifier);
    ...
    fn refund_tokens(
        ...
        refundable_payments: &ManagedVec<DcdtTokenPayment>,
    ) {
    ...
    fn burn_mainchain_token(
        ...
        if payment.token_nonce > 0 {
            ...
            if self.is_nft(payment_token_type) {
                self.clear_mvx_to_sov_dcdt_info_mapper(
                    &payment.token_identifier,
                    payment.token_nonce,
                );

                self.clear_sov_to_mvx_dcdt_info_mapper(sov_token_id, sov_token_nonce);
            }
        }
