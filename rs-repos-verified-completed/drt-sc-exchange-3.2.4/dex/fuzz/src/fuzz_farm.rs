...
pub mod fuzz_farm_test {
        ...
        PriceDiscObjBuilder: 'static + Copy + Fn() -> price_discovery::ContractObj<DebugApi>,
    {
        ...
        let lp_token_before =
            fuzzer_data
                .blockchain_wrapper
                .get_dcdt_balance(&caller.address, lp_token_id, 0);
        ...
        if merge_farm_positions && farm_setup.farmer_info.get(&caller.address).is_some() {
            for farm_token_nonce in farm_setup.farmer_info.get(&caller.address).unwrap().iter() {
                let farm_token_amount = fuzzer_data.blockchain_wrapper.get_dcdt_balance(
                    &caller.address,
                    farm_token_id,
                    *farm_token_nonce,
                );
        ...
        PriceDiscObjBuilder: 'static + Copy + Fn() -> price_discovery::ContractObj<DebugApi>,
    {
        ...
        let farm_token_before = fuzzer_data.blockchain_wrapper.get_dcdt_balance(
            &caller.address,
            farm_token_id,
            farm_token_nonce,
        );
        ...
        let reward_token_before =
            fuzzer_data
                .blockchain_wrapper
                .get_dcdt_balance(&caller.address, reward_token_id, 0);
        ...
        if tx_result_string.trim().is_empty() {
            ...
            // Check rewards
            let reward_token_after = fuzzer_data.blockchain_wrapper.get_dcdt_balance(
                &caller.address,
                reward_token_id,
                0,
            );
