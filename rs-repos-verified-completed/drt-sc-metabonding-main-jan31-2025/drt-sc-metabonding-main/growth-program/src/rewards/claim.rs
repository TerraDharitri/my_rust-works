    ...
    + dharitri_sc_modules::pause::PauseModule
{
    ...
    ) -> OptionalValue<DcdtTokenPayment> {
    ...
    fn claim_rewards_normal(&self, args: ClaimRewardsArgs<Self::Api>) -> DcdtTokenPayment {
        ...
        let unlocked_payment = DcdtTokenPayment::new(args.reward_token_id, 0, user_rewards);
        if lock_period > 0 {
            self.lock_tokens(unlocked_payment, lock_period, args.caller.clone())
        } else {
            self.send()
                .direct_non_zero_dcdt_payment(args.caller, &unlocked_payment);
    ...
    fn lock_tokens(
        ...
        let output_payment: RewaOrDcdtTokenPayment = self
        ...
        DcdtTokenPayment::new(
            output_payment.token_identifier.unwrap_dcdt(),
