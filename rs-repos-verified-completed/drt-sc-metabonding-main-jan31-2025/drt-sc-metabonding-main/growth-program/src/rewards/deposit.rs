    ...
    + energy_query::EnergyQueryModule
{
    ...
    fn deposit_rewards_common(
        ...
        let surplus_payment =
            DcdtTokenPayment::new(payment.token_identifier.clone(), 0, surplus_amount);
        self.send()
            .direct_non_zero_dcdt_payment(&caller, &surplus_payment);

        DcdtTokenPayment::new(payment.token_identifier, 0, total_rewards)
    }
}
