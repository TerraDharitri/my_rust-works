...
fn deposit_rewards_test() {
    ...
    mb_setup.b_mock.set_dcdt_balance(
        &rand_user,
        FIRST_PROJ_TOKEN,
...
fn claim_rewards_test() {
    ...
    mb_setup
        .call_claim_rewards(&first_user_addr, 1, 25_000, 0, &sig_first_user_week_1)
        .assert_ok();
    mb_setup.b_mock.check_dcdt_balance(
        &first_user_addr,
        FIRST_PROJ_TOKEN,
...
fn project_with_split_rewards_test() {
    ...
    // 25% goes to LKMOA holders, 75% to delegation
    mb_setup
        .call_add_project(
            FIRST_PROJ_ID,
            &mb_setup.first_project_owner.clone(),
