...
pub struct Project<M: ManagedTypeApi> {
    ...
    pub lkmoa_reward_supply: BigUint<M>,
    pub start_week: Week,
    pub end_week: Week,
}
...
pub trait ProjectModule: crate::common_storage::CommonStorageModule {
    ...
    /// - lkmoa_rewards_percentage - The percentage of the total rewards which will be given to LKMOA stakers.
    ...
    fn add_project(
        ...
        start_week: Week,
        duration_weeks: Week,
        lkmoa_rewards_percentage: u64,
    ) {
        require!(
            reward_token.is_valid_dcdt_identifier(),
        ...
        require!(
            lkmoa_rewards_percentage <= MAX_PERCENTAGE,
            "Invalid percentage"
        );
        ...
        let lkmoa_reward_supply = &reward_supply * lkmoa_rewards_percentage / MAX_PERCENTAGE;
        let delegation_reward_supply = &reward_supply - &lkmoa_reward_supply;
    ...
    fn clear_expired_projects(&self) -> OperationCompletionStatus {
        let mut prev_token = TokenIdentifier::from_dcdt_bytes(&[]);
    ...
    fn clear_and_refund_project(
        ...
        if leftover_funds > 0 {
            self.send()
                .direct_dcdt(&project_owner, token_id, 0, &leftover_funds);
        }
    }
