...
pub struct ClaimArgsWrapper<M: ManagedTypeApi> {
    ...
    pub user_lkmoa_staked_amount: BigUint<M>,
    ...
    + sc_whitelist_module::SCWhitelistModule
{
    ...
    fn collect_claim_args(
        ...
        for raw_arg in raw_claim_args {
            let (week, user_delegation_amount, user_lkmoa_staked_amount, signature) =
                raw_arg.into_tuple();
            let checkpoint = self
                .rewards_checkpoints()
    ...
    fn claim_for_project(
        ...
        let reward_payment = DcdtTokenPayment::new(project.reward_token, 0, rewards_for_project);
        Some(reward_payment)
    }
