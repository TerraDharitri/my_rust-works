use account_abstraction::{
    common::{
        common_types::{PaymentsVec, REWA_TOKEN_ID},
        signature::Signature,
        users::UsersModule,
    },
    AccountAbstraction,
};
use dharitri_sc::types::{Address, DcdtTokenPayment};
...
pub const FIRST_USER_REWA_BALANCE: u64 = 500;
pub const FIRST_USER_DCDT_BALANCE: u64 = 1_000;
pub const SECOND_USER_DCDT_BALANCE: u64 = 2_000;
    ...
    AbstractionBuilder: 'static + Copy + Fn() -> account_abstraction::ContractObj<DebugApi>,
{
    pub fn new(abstraction_builder: AbstractionBuilder) -> Self {
        ...
        let first_user = b_mock.create_user_account(&rust_biguint!(FIRST_USER_REWA_BALANCE));
        let second_user = b_mock.create_user_account(&rust_zero);
        b_mock.set_dcdt_balance(
            &first_user,
            TOKEN_ID,
            &rust_biguint!(FIRST_USER_DCDT_BALANCE),
        );
        b_mock.set_dcdt_balance(
            &second_user,
            TOKEN_ID,
            &rust_biguint!(SECOND_USER_DCDT_BALANCE),
        );
        ...
        // first user deposit REWA
        b_mock
            .execute_tx(
                &first_user,
                &sc_wrapper,
                &rust_biguint!(FIRST_USER_REWA_BALANCE),
                |sc| {
        ...
        b_mock
            .execute_query(sc_wrapper, |sc| {
                ...
                for expected_token in expected_tokens {
                    expected_tokens_managed.push(DcdtTokenPayment::new(
                        managed_token_id!(expected_token.token_identifier.clone()),
                        expected_token.nonce,
                        dharitri_sc::types::BigUint::from_bytes_be(
