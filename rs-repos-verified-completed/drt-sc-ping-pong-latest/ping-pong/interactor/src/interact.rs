...
impl PingPongInteract {
    ...
    pub async fn accepted_payment_token_id(&mut self) -> String {
        ...
        if result_value.is_rewa() {
            return REWA.to_owned();
        }
...
fn get_token_identifier(token_id: String) -> RewaOrDcdtTokenIdentifier<StaticApi> {
    if token_id.to_uppercase().eq(REWA) {
