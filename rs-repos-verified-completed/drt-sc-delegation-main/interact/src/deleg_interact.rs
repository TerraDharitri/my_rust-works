...
impl AdderInteract {
    ...
    async fn query_total_active_stake(&mut self) {
        ...
        println!("WithdrawOnly:    {}", display_rewa_amount(&tuple.0));
        println!("Waiting:         {}", display_rewa_amount(&tuple.1));
        println!("Active:          {}", display_rewa_amount(&tuple.2));
        println!("UnStaked:        {}", display_rewa_amount(&tuple.3));
        println!("DeferredPayment: {}", display_rewa_amount(&tuple.4));
    }
    ...
    async fn query_user_stake_by_type(&mut self, address: &Bech32Address) {
        ...
        println!("WithdrawOnly:      {}", display_rewa_amount(&withdraw));
        println!("Waiting:           {}", display_rewa_amount(&waiting));
        println!("Active:            {}", display_rewa_amount(&active));
        println!("UnStaked:          {}", display_rewa_amount(&unstaked));
        println!("DeferredPayment:   {}", display_rewa_amount(&deferred));
        ...
        println!(
            "Total Unstakeable: {}",
            display_rewa_amount(&total_unstakeable)
        );
        ...
        println!(
            "Total Unbondable:  {}",
            display_rewa_amount(&total_unbondable)
        );
    ...
    async fn query_all_user_stake_by_type(&mut self) {
        ...
        for user_stake_tuple in result {
            ...
            print!("{address}: ");
            print!(" {}", display_rewa_amount(&stake_by_type.0));
            print!(" {}", display_rewa_amount(&stake_by_type.1));
            print!(" {}", display_rewa_amount(&stake_by_type.2));
            print!(" {}", display_rewa_amount(&stake_by_type.3));
            print!(" {}", display_rewa_amount(&stake_by_type.4));
            println!();
        }
    }
}

fn display_rewa_amount(managed_bu: &BigUint<StaticApi>) -> String {
    if managed_bu == &0u32 {
        return "       0".to_string();
    }
