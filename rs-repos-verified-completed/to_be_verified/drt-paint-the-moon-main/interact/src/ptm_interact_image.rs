...
impl MoonInteract {
    pub async fn compose_image(&mut self) -> anyhow::Result<Vec<Point>> {
        ...
        let image = ImageReader::open("rewa-logo.png")?.decode()?;
        println!("image size: {} x {}", image.width(), image.height());
        let rewa_logo = image.resize(100, 100, image::imageops::FilterType::Nearest);
        println!("resized:    {} x {}", rewa_logo.width(), rewa_logo.height());

        let changed_points = add_overlay(&rewa_logo, 512, 128, &map).await?;
