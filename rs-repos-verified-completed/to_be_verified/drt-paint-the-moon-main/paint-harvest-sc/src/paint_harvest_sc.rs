    ...
    + events::EventsModule
{
    ...
    fn claim_harvest(&self) {
        ...
        for color in self.colors_harvested(&caller).iter() {
            ...
            all_rewards.push(DcdtTokenPayment::new(
        ...
        self.tx().to(&caller).multi_dcdt(all_rewards).transfer();
    ...
    fn stop_harvest(&self) {
        ...
        // claim rewards + accumulated rewards
        let mut all_rewards = ManagedVec::<Self::Api, DcdtTokenPayment<Self::Api>>::new();
        let paint_token_id = self.paint_token_id().get();
