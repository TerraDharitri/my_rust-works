...
impl AdderInteract {
    ...
    pub async fn multi_deploy(&mut self) {
        ...
        for block_size in BLOCK_SIZES {
            ...
            let code_path = format!("../paint-the-moon-sc/output/block-{block_size}.drtsc.json");

            buffer.push_tx(|tx| {
                tx.from(&self.owner_address)
                    .typed(moon_proxy::PaintTheMoonScProxy)
                    .init()
                    .code(DrtscPath::new(&code_path))
                    .gas(10_000_000)
    ...
    pub async fn multi_upgrade(&mut self) {
        ...
        for contract_info in &self.state.contract_info_list {
            ...
            let code_path =
                format!("../paint-the-moon-sc/output/paint-the-moon-{block_size}.drtsc.json");
