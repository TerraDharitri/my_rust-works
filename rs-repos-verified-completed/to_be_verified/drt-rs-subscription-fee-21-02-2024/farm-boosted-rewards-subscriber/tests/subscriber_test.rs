...
static WREWA_TOKEN_ID: &[u8] = b"WREWA-123456";
static LP_TOKEN_ID: &[u8] = b"LPTOK-123456";
static STABLE_LP_TOKEN_ID: &[u8] = b"SLPTOK-123456";
static REWARD_TOKEN_ID: &[u8] = b"MOA-123456";
static LOCKED_TOKEN_ID: &[u8] = b"XMOA-123456";
...
>(
    ...
    let moa_pair_setup = PairSetup::new(
        b_mock_rc.clone(),
        pair_builder,
        &owner,
        WREWA_TOKEN_ID,
    ...
    let farm_setup = FarmSetup::new(
        ...
        moa_pair_setup.pair_wrapper.address_ref(),
        farm_builder,
...
fn claim_boosted_rewards_for_user_test() {
    ...
    b_mock_rc
        .borrow_mut()
        .set_dcdt_balance(&user, WREWA_TOKEN_ID, &rust_biguint!(1_000_000));

    subscription_setup
        .call_deposit(&user, WREWA_TOKEN_ID, 1_000_000)
        .assert_ok();
    ...
    b_mock_rc
        .borrow_mut()
        .set_dcdt_balance(&user, LP_TOKEN_ID, &rust_biguint!(user_token_amount));
    ...
    b_mock_rc.borrow().check_dcdt_balance(
...
fn claim_boosted_rewards_for_user_multiple_farms_test() {
    ...
    // Generate farm rewards
    let user_token_amount = 100_000_000;
    b_mock_rc.borrow_mut().set_dcdt_balance(
        &user,
        LP_TOKEN_ID,
...
#[test]
fn moa_operation_with_claim_fees_test() {
    let (
        b_mock_rc,
        _moa_pair_setup,
        _stable_pair_setup,
    ...
    b_mock_rc
        .borrow_mut()
        .set_dcdt_balance(&first_user, WREWA_TOKEN_ID, &rust_biguint!(1_000_000));

    subscription_setup
        .call_deposit(&first_user, WREWA_TOKEN_ID, 1_000_000)
        .assert_ok();
    ...
    subscription_setup
        .call_deposit(&second_user, WREWA_TOKEN_ID, 1_000_000)
        .assert_ok();
    ...
    subscriber_setup
        .call_perform_moa_operation(STANDARD_SERVICE, vec![first_user_id, second_user_id])
        .assert_ok();
...
fn subtract_worth_of_stable_payment_test() {
    ...
    // Pool ratio is 1:2, so for a service payment of 500 USDC worth of WREWA, the amount should be 250
    subscriber_setup
        .call_subtract_payment(vec![user_id])
        .assert_ok();
