...
pub trait EventsModule {
    ...
    fn emit_moa_operation_event(self, service_index: usize, user_ids: ManagedVec<AddressId>) {
        let caller = self.blockchain().get_caller();
        let epoch = self.blockchain().get_block_epoch();
        self.moa_operation_event(caller, epoch, service_index, user_ids)
    }
    ...
    #[event("moaOperationEvent")]
    fn moa_operation_event(
        self,
        #[indexed] caller: ManagedAddress,
        #[indexed] epoch: u64,
        #[indexed] service_index: usize,
