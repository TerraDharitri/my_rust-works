...
pub trait PairActionsModule: crate::common_storage::CommonStorageModule {
    ...
    fn add_pair_address(&self, payment_token_id: TokenIdentifier, pair_address: ManagedAddress) {
        require!(
            payment_token_id.is_valid_dcdt_identifier(),
            "Invalid token ID"
        );
        require!(
    ...
    fn get_worth_of_price(
        ...
        let stable_pair_data_mapper = self.pair_address_for_token(&wrewa_token_id);
        if stable_pair_data_mapper.is_empty() {
            return Result::Err(());
        }
        let stable_pair_address = stable_pair_data_mapper.get();
        let wrewa_price: DcdtTokenPayment = self
            .pair_proxy(price_query_address.clone())
            .get_safe_price_by_default_offset(
                stable_pair_address,
                DcdtTokenPayment::new(stable_token_id, 0, stable_worth_amount),
            )
            .execute_on_dest_context();

        if desired_token_id == &wrewa_price.token_identifier {
            return Result::Ok(wrewa_price.amount);
        }
        ...
        let price: DcdtTokenPayment = self
            .pair_proxy(price_query_address)
            .get_safe_price_by_default_offset(pair_address, wrewa_price)
            .execute_on_dest_context();
