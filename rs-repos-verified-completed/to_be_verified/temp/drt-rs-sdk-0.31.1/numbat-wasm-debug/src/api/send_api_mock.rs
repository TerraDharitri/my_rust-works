...
use numbat_wasm::{
    api::{
        BlockchainApiImpl, ManagedTypeApi, SendApi, SendApiImpl, DCDT_MULTI_TRANSFER_FUNC_NAME,
        DCDT_NFT_TRANSFER_FUNC_NAME, DCDT_TRANSFER_FUNC_NAME, UPGRADE_CONTRACT_FUNC_NAME,
    },
    numbat_codec::top_encode_to_vec_u8,
    err_msg,
    types::{
        heap::Address, BigUint, CodeMetadata, DcdtTokenPayment, ManagedAddress, ManagedArgBuffer,
...
impl SendApiImpl for DebugApi {
        ...
        D: Into<ManagedBuffer<M>>,
    {
        ...
        let contract_address = &self.input_ref().to;
        self.blockchain_cache()
            .subtract_rewa_balance(contract_address, &amount_value);
