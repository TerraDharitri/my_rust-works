    ...
    crate::default_issue_callbacks::DefaultIssueCallbacksModule + crate::pause::PauseModule
{
    ...
    fn merge_tokens(&self) -> DcdtTokenPayment<Self::Api> {
    ...
    fn split_tokens(&self) -> MultiValueEncoded<DcdtTokenPayment<Self::Api>> {
        self.require_not_paused();
        ...
        for token in &payments {
            ...
            for inst in previously_merged_instance_attributes {
                let original_token = DcdtTokenPayment::new(
                    TokenIdentifier::from_dcdt_bytes(inst.original_token_id_raw.as_slice()),
    ...
    fn split_token_partial(
        &self,
        tokens_to_remove: MultiValueEncoded<DcdtTokenPayment<Self::Api>>,
    ) -> MultiValueEncoded<DcdtTokenPayment<Self::Api>> {
        self.require_not_paused();

        let merged_token = self.call_value().single_dcdt();
        self.merged_token()
