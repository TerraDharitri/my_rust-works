use dharitri_sc::{
    codec::Empty,
    types::{Address, BigUint, DcdtLocalRole},
};
...
use crate::{
    constants::{
        ACCOUNT_TOKEN, REWA_TOKEN_ID, LIQ_THRESOLD, RESERVE_FACTOR, R_BASE, R_SLOPE1, R_SLOPE2,
    ...
    PriceAggregatorObjBuilder: 'static + Copy + Fn() -> aggregator_mock::ContractObj<DebugApi>,
{
    ...
    pub liquidity_pool_rewa_wrapper:
        ContractObjWrapper<liquidity_pool::ContractObj<DebugApi>, LiquidityPoolObjBuilder>,
}
    ...
    PriceAggregatorObjBuilder: 'static + Copy + Fn() -> aggregator_mock::ContractObj<DebugApi>,
{
    ...
    pub fn deploy_lending(
        ...
        let liquidity_pool_rewa_wrapper = b_mock.create_sc_account(
            &rust_biguint!(0u64),
            Some(&lending_pool_wrapper.address_ref()),
            liquidity_pool_builder,
            "liq_pool_template_other",
        ...
        b_mock
            .execute_tx(
                ...
                |sc| {
                    sc.pools_map().insert(
                        managed_token_id!(REWA_TOKEN_ID),
                        managed_address!(&liquidity_pool_rewa_wrapper.address_ref()),
                    );
                    sc.pools_allowed()
                        .insert(managed_address!(&liquidity_pool_rewa_wrapper.address_ref()));
        ...
        Self {
            ...
            liquidity_pool_rewa_wrapper: liquidity_pool_rewa_wrapper,
        }
    }
    ...
    pub fn add_collateral(
        ...
        let liquidity_pool_wrapper = match token_id {
            ...
            REWA_TOKEN_ID => &self.liquidity_pool_rewa_wrapper,
            _ => todo!(),
        };
