    ...
    fn check_dcdt_balance(
        ...
        self.world
            .check_state_step(CheckStateStep::new().put_account(
                address_expr,
                CheckAccount::new().dcdt_balance(token_id_expr, balance_expr),
            ));
    ...
    state
        ...
        .sc_call(
            ScCallStep::new()
                ...
                .dcdt_transfer(FEE_TOKEN_ID_EXPR, 0, FEE_AMOUNT)
        )
        .check_state_step(CheckStateStep::new().put_account(
            RELAYER_ADDRESS_EXPR,
            CheckAccount::new().dcdt_balance(FEE_TOKEN_ID_EXPR, FEE_AMOUNT),
        ));
}
...
fn test_forward_call_sc_adder() {
    ...
    state.world.sc_call(
        ScCallStep::new()
            .from(CALLER_ADDRESS_EXPR)
            .dcdt_transfer(FEE_TOKEN_ID_EXPR, 0, FEE_AMOUNT)
            .dcdt_transfer(PAYMASTER_TOKEN_ID_EXPR, 0, FEE_AMOUNT)
            .call(state.paymaster_contract.forward_execution(
                state.relayer_address.clone(),
...
fn test_forward_call_sc_adder_multiple_payments() {
    ...
    state.world.sc_call(
        ScCallStep::new()
            ...
            .dcdt_transfer(FEE_TOKEN_ID_EXPR, 0, FEE_AMOUNT)
            .dcdt_transfer(PAYMASTER_TOKEN_ID_EXPR, 0, FEE_AMOUNT)
            .dcdt_transfer(ADDITIONAL_TOKEN_ID_EXPR, 0, FEE_AMOUNT)
            .call(state.paymaster_contract.forward_execution(
                state.relayer_address.clone(),
                state.callee_sc_adder_contract.to_address(),
                b"add",
