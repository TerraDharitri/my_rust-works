use liquid_locking::*;
use dharitri_sc::types::{BigUint, DcdtTokenPayment, ManagedVec, TokenIdentifier};
...
fn test() {
    ...
    world
        .set_state_step(
            SetStateStep::new().put_account(
                user_1,
                Account::new()
                    .dcdt_balance(whitelisted_token_1, 1_000u64)
                    .dcdt_balance(whitelisted_token_2, 1_000u64),
            ),
        )
        .set_state_step(
            SetStateStep::new().put_account(
                user_2,
                Account::new()
                    .dcdt_balance(blacklisted_token, 1_000u64)
                    .dcdt_balance(whitelisted_token_2, 1_000u64)
    ...
    world
        .sc_call_step(
            ScCallStep::new()
                .from(user_2)
                .to(&contract)
                .dcdt_transfer(blacklisted_token, 0u64, 500u64)
                .call(contract.lock())
    ...
    world
        .sc_call_step(
            ScCallStep::new()
                .from(user_2)
                .to(&contract)
                .dcdt_transfer(whitelisted_token_2, 0u64, 500u64)
                .call(contract.lock())
                .expect(TxExpect::ok().no_result()),
        )
        .sc_call_step(
        ...
        .sc_call_step(
            ScCallStep::new()
                .from(user_1)
                .to(&contract)
                .dcdt_transfer(whitelisted_token_1, 0u64, 1_000u64)
                .dcdt_transfer(whitelisted_token_2, 0u64, 1_000u64)
                .call(contract.lock())
                .expect(TxExpect::ok().no_result()),
        )
        .check_state_step(
            CheckStateStep::new()
                ...
                .put_account(
                    ...
                    CheckAccount::new()
                        .dcdt_balance(whitelisted_token_1, 1_000u64)
                        .dcdt_balance(whitelisted_token_2, 2_000u64)
                        .check_storage("str:unbond_period", "10")
                        .check_storage("str:token_whitelist.len", "2")
                        .check_storage("str:token_whitelist.item|u32:1", "str:AAA-111111")
                        .check_storage("str:token_whitelist.item|u32:2", "str:BBB-222222")
    ...
    let mut unlock_single_dcdt = ManagedVec::<DebugApi, DcdtTokenPayment<DebugApi>>::new();
    let mut unlock_multiple_dcdt = ManagedVec::<DebugApi, DcdtTokenPayment<DebugApi>>::new();
    ...
    world
        .sc_call_step(
            ScCallStep::new()
                .from(user_2)
                .to(&contract)
                .call(contract.unlock(unlock_single_dcdt))
        ...
        .sc_call_step(
            ScCallStep::new()
                .from(user_2)
                .to(&contract)
                .call(contract.unlock(unlock_multiple_dcdt))
                .expect(TxExpect::err(4, "str:requested amount cannot be 0")),
        );

    // unlock success

    unlock_single_dcdt = ManagedVec::<DebugApi, DcdtTokenPayment<DebugApi>>::new();
    unlock_multiple_dcdt = ManagedVec::<DebugApi, DcdtTokenPayment<DebugApi>>::new();
    ...
    world
        .sc_call_step(
            ScCallStep::new()
                .from(user_2)
                .to(&contract)
                .call(contract.unlock(unlock_single_dcdt.clone()))
                .expect(TxExpect::ok().no_result()),
        )
        .sc_call_step(
            ScCallStep::new()
        ...
        .check_state_step(
            CheckStateStep::new()
                ...
                .put_account(
                    ...
                    CheckAccount::new()
                        .dcdt_balance(whitelisted_token_1, 1_000u64)
                        .dcdt_balance(whitelisted_token_2, 700u64),
                )
                .put_account(
                    user_2,
                    CheckAccount::new()
                        .dcdt_balance(blacklisted_token, 1_000u64)
                        .dcdt_balance(whitelisted_token_2, 200u64)
                        .balance("1_000_000_000"),
                )
                .put_account(
                    &contract,
                    CheckAccount::new()
                        .dcdt_balance(whitelisted_token_2, 1_100u64)
                        .check_storage("str:unbond_period", "10")
                        .check_storage("str:token_whitelist.len", "2")
                        .check_storage("str:token_whitelist.item|u32:1", "str:AAA-111111")
                        .check_storage("str:token_whitelist.item|u32:2", "str:BBB-222222")
