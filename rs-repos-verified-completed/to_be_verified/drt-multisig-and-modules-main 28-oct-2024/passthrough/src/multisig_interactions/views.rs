...
pub trait MultisigInteractionsViews: super::storage::MultisigInteractionsStorage {
    ...
    fn can_execute(
        ...
        self.is_token_allowed(sc_id, &endpoint_name, &rewa_value, &dcdt_payments)
    }
    ...
    fn get_allowed_token_for_interaction(
        ...
        sc_address: ManagedAddress,
        endpoint_name: ManagedBuffer,
    ) -> OptionalValue<RewaOrDcdtTokenIdentifier> {
        let sc_id = self.sc_address_to_id().get_id_non_zero(&sc_address);
    ...
    fn is_token_allowed(
        ...
        match opt_allowed_token {
            Some(allowed_token) => {
                if rewa_value > &0 && !allowed_token.is_rewa() {
                    return false;
                }

                for payment in dcdt_payments {
            ...
            None => {
                if rewa_value > &0 || !dcdt_payments.is_empty() {
                    return false;
                }
            }
        }
