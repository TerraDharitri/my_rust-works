...
use crate::common_types::{
    action::{
        Action, ActionId, ActionStatus, CallActionData, DeployArgs, DcdtTransferExecuteData,
        GasLimit, GroupId,
    },
    ...
    + crate::external::events::EventsModule
{
    ...
    fn propose_transfer_execute(
        ...
        let action = Action::SendTransferExecuteRewa(call_data.clone());
        let action_id = self.add_action(&action);

        if self.try_perform_rewa_action_directly(&proposer, action_id, &call_data) {
    ...
    fn propose_transfer_execute_dcdt(
        ...
        let action = Action::SendTransferExecuteDcdt(call_data.clone());
        let action_id = self.add_action(&action);

        if self.try_perform_dcdt_action_directly(&proposer, action_id, &call_data) {
            return OptionalValue::None;
        }
