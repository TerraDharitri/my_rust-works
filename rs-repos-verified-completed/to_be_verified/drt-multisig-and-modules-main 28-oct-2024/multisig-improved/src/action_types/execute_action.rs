use crate::common_types::{
    action::{Action, ActionId, CallActionData, DeployArgs, DcdtTransferExecuteData, GasLimit},
    ...
    + crate::ms_endpoints::callbacks::CallbacksModule
{
    ...
    fn execute_external_call(&self, action_id: ActionId, action: Action<Self::Api>) {
        match action {
            Action::SendTransferExecuteRewa(call_data) => {
                self.send_transfer_execute_rewa(action_id, call_data);
            }
            Action::SendTransferExecuteDcdt(call_data) => {
                self.send_transfer_execute_dcdt(action_id, call_data);
            }
    ...
    fn send_transfer_execute_dcdt(
        &self,
        action_id: ActionId,
        call_data: DcdtTransferExecuteData<Self::Api>,
    ) {
        let gas = call_data
            .opt_gas_limit
            .unwrap_or_else(|| self.ensure_and_get_gas_for_transfer_exec());
