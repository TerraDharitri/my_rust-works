...
fn transfer_execute_to_user_test() {
    ...
    let rewa_balance = 100;
    ms_setup
        .b_mock
        .set_rewa_balance(&ms_setup.first_board_member, &rust_biguint!(rewa_balance));
    ...
    // propose
    let action_id = ms_setup.propose_transfer_execute(&new_user, rewa_balance, &[], Vec::new());
    ms_setup.sign(action_id, 0);
    ms_setup.perform(action_id);

    ms_setup
        .b_mock
        .check_rewa_balance(ms_setup.ms_wrapper.address_ref(), &rust_biguint!(0));
    ms_setup
        .b_mock
        .check_rewa_balance(&new_user, &rust_biguint!(rewa_balance));
}
    ...
    ms_setup
        ...
        .execute_tx(
            ...
            |sc| {
                ...
                let single_action = Action::SendTransferExecuteRewa(CallActionData {
                    to: managed_address!(&adder_addr),
                    rewa_amount: managed_biguint!(0),
                    opt_gas_limit: None,
...
#[test]
fn execute_async_call_to_sc_test() {
    let mut ms_setup = MsImprovedSetup::new(multisig_improved::contract_obj, adder::contract_obj);
...
#[test]
fn execute_sync_call_to_sc_test() {
    let mut ms_setup = MsImprovedSetup::new(multisig_improved::contract_obj, adder::contract_obj);

    let args = [&[5u8][..]].to_vec();
    let action_id = ms_setup.propose_sync_call(
        &ms_setup.adder_wrapper.address_ref().clone(),
        0,
        b"add",
        args,
    ...
    ms_setup
        ...
        .execute_tx(
            ...
            |sc| {
                ...
                add_function_args
                    .push_arg::<dharitri_sc::types::BigUint<DebugApi>>(managed_biguint!(5));
