...
pub trait TokenWhitelistModule {
    ...
    fn add_tokens_to_whitelist(&self, tokens_to_add: MultiValueEncoded<RewaOrDcdtTokenIdentifier>) {
    ...
    fn remove_tokens_from_whitelist(
        &self,
        tokens_to_remove: MultiValueEncoded<RewaOrDcdtTokenIdentifier>,
    ) {
        let mut whitelisted_tokens_mapper = self.whitelisted_tokens();
    ...
    fn require_token_whitelisted(&self, token_id: &RewaOrDcdtTokenIdentifier) {
    ...
    fn whitelisted_tokens(&self) -> UnorderedSetMapper<RewaOrDcdtTokenIdentifier>;
}
