...
pub struct Offer<M: ManagedTypeApi> {
    pub offer_token: DcdtTokenPayment<M>,
    ...
    + numbat_wasm_modules::pause::PauseModule
{
    ...
    fn send_offer(
        ...
        let offer_token = DcdtTokenPayment::new(desired_nft_id, desired_nft_nonce, desired_amount);
    ...
    fn check_nft_in_marketplace(
        ...
        let token_amount_in_marketplace = self.blockchain().get_sc_balance(
            &RewaOrDcdtTokenIdentifier::dcdt(desired_nft_id.clone()),
            desired_nft_nonce,
        );
    ...
    fn accept_offer(&self, offer_id: u64) {
        self.require_not_paused();
        let caller = self.blockchain().get_caller();
        let offer_nft = self.call_value().single_dcdt();
        let offer = self.try_get_offer(offer_id);
        require!(
