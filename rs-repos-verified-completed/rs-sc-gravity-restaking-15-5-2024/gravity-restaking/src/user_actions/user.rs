    ...
    + utils::UtilsModule
{
    ...
    fn withdraw(&self, tokens: PaymentsMultiValue<Self::Api>) {
        ...
        let mut total_rewa = BigUint::zero();
        self.user_tokens(caller_id).update(|user_tokens| {
            for token_tuple in tokens {
                ...
                let payment = DcdtTokenPayment::new(token_id, nonce, amount);
                ...
                if payment.token_identifier != rewa_token_id {
                    ...
                    total_rewa += payment.amount;
                }
            }
        });

        self.send().direct_non_zero_rewa(&caller, &total_rewa);
    ...
    fn withdraw_all(&self) {
        ...
        if let Some(index_to_remove) = opt_index_to_remove {
            let rewa_payment = output_payments.get(index_to_remove);
            output_payments.remove(index_to_remove);

            self.send().direct_rewa(&caller, &rewa_payment.amount);
        }
