use crate::user_actions::call_delegation::REWA_TOKEN_ID;
...
pub trait TokenWhitelistModule {
    ...
    fn add_token_to_whitelist(
        ...
        staked_rewa_for_one_token: BigUint,
        opt_custom_token_decimals: OptionalValue<usize>,
    ) {
        self.staked_rewa_for_one_token(&token_id)
            .set(staked_rewa_for_one_token);
    ...
    /// Note: Don't remove and add to set the staked_rewa_for_one_token parameter, you'll ruin the internal consistency
    #[only_owner]
    ...
    fn remove_token_from_whitelist(&self, token_id: TokenIdentifier) {
        ...
        self.staked_rewa_for_one_token(&token_id).clear();
    ...
    fn get_total_staked_rewa(&self, token_id: &TokenIdentifier, amount: &BigUint) -> BigUint {
        if token_id == &TokenIdentifier::from_dcdt_bytes(REWA_TOKEN_ID) {
            return amount.clone();
        }

        let staked_rewa_one_token = self.staked_rewa_for_one_token(token_id).get();
        let decimals = self.get_token_decimals(token_id);

        staked_rewa_one_token * amount / BigUint::from(BASE_FOR_DECIMALS).pow(decimals as u32)
    }
    ...
    #[view(getStakedRewaForOneToken)]
    #[storage_mapper("stkRewaTok")]
    fn staked_rewa_for_one_token(&self, token_id: &TokenIdentifier) -> SingleValueMapper<BigUint>;
