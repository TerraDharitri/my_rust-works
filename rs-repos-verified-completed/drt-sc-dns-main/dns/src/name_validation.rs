...
const NUMBAT_SUFFIX: &[u8] = b".numbat";
...
// switch the suffix from .x to .numbat for the purpose of name hashing
// all other names (eg. using the .numbat suffix, or some invalid names), are not modified
pub fn prepare_name_for_hash_and_classify<M: ManagedTypeApi>(
    name: &ManagedBuffer<M>,
) -> (ManagedBuffer<M>, SuffixType) {
    try_replace_suffix(name, X_SUFFIX, NUMBAT_SUFFIX).map_or_else(
        |_| (name.clone(), SuffixType::Numbat),
...
// ensure the suffix is .numbat and replace it with .x
// validate the name without the prefix as well
pub fn prepare_and_validate_name_for_migration<M: ManagedTypeApi>(
    name: &ManagedBuffer<M>,
) -> Result<ManagedBuffer<M>, &'static str> {
    let name_cache = NameCache::try_load(name)?;
    let name_without_suffix = name_cache.check_suffix_and_get_name(NUMBAT_SUFFIX)?;
    validate_name_without_suffix(name_without_suffix)?;
