...
fn unlock_token_test() {
    ...
    DebugApi::dummy();
    b_mock.set_dcdt_balance(sc_wrapper.address_ref(), FREE_TOKEN_ID, &lock_amount);
    ...
    b_mock.check_dcdt_balance(sc_wrapper.address_ref(), FREE_TOKEN_ID, &lock_amount);
    b_mock.check_dcdt_balance(&user_addr, FREE_TOKEN_ID, &rust_zero);
    ...
    b_mock
        ...
        .assert_ok();
    b_mock.check_dcdt_balance(&user_addr, FREE_TOKEN_ID, &lock_amount);
    b_mock.check_nft_balance(
        &user_addr,
        LOCKED_TOKEN_ID,
        lock_token_nonce,
    ...
    b_mock.check_dcdt_balance(sc_wrapper.address_ref(), FREE_TOKEN_ID, &rust_zero);
    b_mock.check_nft_balance(
        sc_wrapper.address_ref(),
        LOCKED_TOKEN_ID,
        lock_token_nonce,
...
fn exit_lp_test() {
    ...
    b_mock.set_dcdt_balance(sc_wrapper.address_ref(), LP_TOKEN_ID, &lock_amount);

    b_mock
        .execute_tx(&owner_addr, &sc_wrapper, &rust_zero, |sc| {
            let lp_proxy_token_amount = managed_biguint!(1_000u64);
            // Needed to be able to read the token attributes
            sc.send().dcdt_nft_create_compact(
            ...
            let lp_proxy_token_nonce = sc.send().dcdt_nft_create_compact(
                ...
                &LpProxyTokenAttributes::<DebugApi> {
                    ...
                    second_token_id: managed_token_id!(WREWA_TOKEN_ID),
    ...
    b_mock
        ...
        .assert_ok();
    b_mock.check_dcdt_balance(&user_addr, LP_TOKEN_ID, &lock_amount);
    b_mock.check_dcdt_balance(sc_wrapper.address_ref(), LP_TOKEN_ID, &rust_zero);
    b_mock.check_nft_balance(
        &user_addr,
        LP_PROXY_TOKEN_ID,
        lock_token_nonce,
...
fn exit_farm_test() {
    ...
    b_mock
        .execute_tx(&owner_addr, &sc_wrapper, &rust_zero, |sc| {
            ...
            let farm_proxy_token_nonce = sc.send().dcdt_nft_create_compact(
