    ...
    crate::lp_farm_token::LpFarmTokenModule + token_merge_helper::TokenMergeHelperModule
{
    ...
    fn lp_farm_exit(
        ...
        let raw_results: RawResultsType<Self::Api> = self
            ...
            .with_dcdt_transfer((lp_farm_token_id, lp_farm_token_nonce, lp_farm_token_amount))
            .execute_on_dest_context();

        let mut results_wrapper = RawResultWrapper::new(raw_results);
        results_wrapper.trim_results_front(2);

        let mut lp_tokens: DcdtTokenPayment = results_wrapper.decode_next_result();
        let mut lp_farm_rewards: DcdtTokenPayment = results_wrapper.decode_next_result();
    ...
    fn staking_farm_unstake(
        ...
        staking_tokens: DcdtTokenPayment<Self::Api>,
    ...
    fn pair_remove_liquidity(
        &self,
        lp_tokens: DcdtTokenPayment<Self::Api>,
        ...
        let pair_first_token_payment: DcdtTokenPayment = results_wrapper.decode_next_result();
        let pair_second_token_payment: DcdtTokenPayment = results_wrapper.decode_next_result();
