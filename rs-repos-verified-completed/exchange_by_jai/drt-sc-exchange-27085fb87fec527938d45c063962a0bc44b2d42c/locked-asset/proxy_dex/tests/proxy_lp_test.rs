...
fn wrapped_same_nonce_lp_token_merge_test() {
    ...
    let liquidity_token_amount = (&first_amount + &second_amount) * rust_biguint!(2u64); // parity 1 REWA -> 2 MOA
...
fn wrapped_different_nonce_lp_token_merge_test() {
    ...
    setup
        .b_mock
        .set_dcdt_balance(&user, MOA_TOKEN_ID, &user_balance);
    ...
    let expected_locked_token_balance =
        &(locked_token_amount - min_locked_lp_token_amount) * rust_biguint!(2); // parity 1 REWA -> 2 MOA
