...
transferNFTTokens() {
    ...
    address_to_send="$(drtpy wallet bech32 --decode $1)"

    drtpy --verbose tx new --recall-nonce \
        --pem=${WALLET_PEM} \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
        --gas-limit=100000000 \
        --receiver=$user_address \
        --data "DCDTNFTTransfer@$sft_token@$3@$4@$address_to_send" \
        --send || return
}
#### LOCKED ASSET ####
...
deployLockedAssetContract() {
    ...
    ADDRESS=$(drtpy data parse --file="deploy-locked-asset-internal.interaction.json" --expression="data['contractAddress']")
...
upgradeLockedAssetContract() {
    ...
    drtpy --verbose contract upgrade $LOCKED_ASSET_FACTORY_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-price=1499999999 \
        --gas-limit=1499999999 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
...
registerLockedAssetToken() {
    ...
    drtpy --verbose contract call ${LOCKED_ASSET_FACTORY_ADDRESS} --recall-nonce \
          --pem=${WALLET_PEM} \
          --proxy=${PROXY} --chain=${CHAIN_ID} \
          --gas-limit=200000000 \
...
setLocalRolesLockedAssetToken() {
    decoded_address="0x$(drtpy wallet bech32 --decode $1)"
...
#   $1 = Address
whitelist() {
    sc_address="0x$(drtpy wallet bech32 --decode $1)"
...
deployProxyContract() {
    ...
    ADDRESS=$(drtpy data parse --file="deploy-proxy-internal.interaction.json" --expression="data['contractAddress']")
...
upgradeProxyContract() {
    ...
    drtpy --verbose contract upgrade $PROXY_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-limit=300000000 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
        --metadata-payable \
...
registerProxyPair() {
    ...
    drtpy --verbose contract call $PROXY_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
...
transferDCDTTokensProxy() {
    method_name="0x$(echo -n 'acceptDcdtPaymentProxy' | xxd -p -u | tr -d '\n')"
    pair_address="0x$(drtpy wallet bech32 --decode $1)"
...
transferNFTTokensProxy() {
    ...
    pair_address="0x$(drtpy wallet bech32 --decode $1)"
    proxy_address="0x$(drtpy wallet bech32 --decode $PROXY_ADDRESS)"
...
deployDistributionContract() {
    locked_asset_address="0x$(drtpy wallet bech32 --decode $LOCKED_ASSET_FACTORY_ADDRESS)"
    asset_token="0x$(echo -n $1 | xxd -p -u | tr -d '\n')"
    ...
    ADDRESS=$(drtpy data parse --file="deploy-distribution-internal.interaction.json" --expression="data['contractAddress']")
...
upgradeDistributionContract() {
    ...
    drtpy --verbose contract upgrade $DISTRIBUTION_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-price=1499999999 \
        --gas-limit=1499999999 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
...
startGlobalOperation() {
    drtpy --verbose contract call $DISTRIBUTION_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-limit=20000000 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
...
setPerUserDistributedLockedAssets() {
    user_address="0x$(drtpy wallet bech32 --decode $2)"
    drtpy --verbose contract call $DISTRIBUTION_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-limit=50000000 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
...
getTemporaryFundsProxy() {
    user_address="0x$(drtpy wallet bech32 --decode $1)"
    asset_token="0x$(echo -n $2 | xxd -p -u | tr -d '\n')"

    drtpy --verbose contract query $PROXY_ADDRESS \
        --proxy=${PROXY} \
...
# params:
getLockedTokenId() {
    drtpy --verbose contract query $LOCKED_ASSET_FACTORY_ADDRESS \
        --proxy=${PROXY} \
...
getLastCommunityDistributionUnlockMilestones() {
    drtpy --verbose contract query $DISTRIBUTION_ADDRESS \
        --proxy=${PROXY} \
