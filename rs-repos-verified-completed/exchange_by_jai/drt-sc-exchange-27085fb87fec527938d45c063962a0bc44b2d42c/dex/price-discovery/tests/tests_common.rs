    ...
    PriceDiscObjBuilder: 'static + Copy + Fn() -> price_discovery::ContractObj<DebugApi>,
{
    ...
    let owner_address = blockchain_wrapper.create_user_account(&rust_biguint!(OWNER_REWA_BALANCE));
    ...
    let prev_owner_balance =
        blockchain_wrapper.get_dcdt_balance(&owner_address, LAUNCHED_TOKEN_ID, 0);
...
{
    let b_wrapper = &mut pd_setup.blockchain_wrapper;
    b_wrapper.execute_dcdt_transfer(
        caller,
        &pd_setup.pd_wrapper,
        ACCEPTED_TOKEN_ID,
        0,
