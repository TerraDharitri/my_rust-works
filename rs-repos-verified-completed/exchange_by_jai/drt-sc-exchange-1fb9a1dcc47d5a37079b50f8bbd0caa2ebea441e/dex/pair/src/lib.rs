...
pub type SwapTokensFixedInputResultType<BigUint> = DcdtTokenPayment<BigUint>;
    ...
    + pausable::PausableModule
{
    ...
    fn add_initial_liquidity(&self) -> AddLiquidityResultType<Self::Api> {
        ...
        self.send()
            .dcdt_local_mint(&storage_cache.lp_token_id, 0, &liq_added);
        self.send()
            .direct_dcdt(&caller, &storage_cache.lp_token_id, 0, &liq_added);
    ...
    fn add_liquidity(
        ...
        require!(
            storage_cache.lp_token_id.is_valid_dcdt_identifier(),
        ...
        self.send()
            .dcdt_local_mint(&storage_cache.lp_token_id, 0, &add_liq_context.liq_added);
    ...
    fn swap_no_fee(&self, token_out: TokenIdentifier, destination_address: ManagedAddress) {
        ...
        let mut storage_cache = StorageCache::new(self);
        let (token_in, _, amount_in) = self.call_value().single_dcdt().into_tuple();
    ...
    fn swap_tokens_fixed_output(
        ...
        let mut storage_cache = StorageCache::new(self);
        let (token_in, _, amount_in_max) = self.call_value().single_dcdt().into_tuple();
        let swap_tokens_order = storage_cache.get_swap_tokens_order(&token_in, &token_out);
