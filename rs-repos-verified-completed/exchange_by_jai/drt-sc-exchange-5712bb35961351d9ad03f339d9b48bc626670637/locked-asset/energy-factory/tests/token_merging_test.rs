...
use dharitri_sc::{
    ...
    types::{
        BigInt, BigUint, RewaOrDcdtTokenIdentifier, RewaOrDcdtTokenPayment, DcdtLocalRole,
        MultiValueEncoded,
    },
};
...
fn token_merging_test() {
    ...
    setup
        .b_mock
        .execute_dcdt_multi_transfer(&first_user, &setup.sc_wrapper, &payments[..], |sc| {
...
fn test_specific_tokens_merge() {
        ...
        .execute_tx(&owner, &sc_wrapper, &rust_zero, |sc| {
            ...
            let _ = sc.lock_and_send(
                ...
                RewaOrDcdtTokenPayment::new(
                    RewaOrDcdtTokenIdentifier::dcdt(managed_token_id!(BASE_ASSET_TOKEN_ID)),
                    0,
    ...
    b_mock
        .execute_dcdt_multi_transfer(&user, &sc_wrapper, &payments, |sc| {
            let _ = sc.merge_tokens_endpoint(OptionalValue::None);
        })
        .assert_ok();
...
fn merge_same_schedule_test() {
    ...
    setup
        .b_mock
        .execute_dcdt_multi_transfer(&user, &setup.sc_wrapper, &payments[..], |sc| {
            let _ = sc.merge_tokens_endpoint(OptionalValue::None);
        })
        .assert_ok();
