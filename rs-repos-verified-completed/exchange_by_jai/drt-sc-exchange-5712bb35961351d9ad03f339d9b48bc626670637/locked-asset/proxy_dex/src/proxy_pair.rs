    ...
    + legacy_token_decode_module::LegacyTokenDecodeModule
{
    ...
    fn add_liquidity_proxy(
    ...
    ) -> MultiValueEncoded<DcdtTokenPayment> {
        ...
        self.send()
            .dcdt_local_mint(&asset_token_id, 0, &asset_amount);
        ...
        if locked_token_leftover.amount > 0 {
            self.send()
                .dcdt_local_burn(&asset_token_id, 0, &locked_token_leftover.amount);
        }
    ...
    fn remove_liquidity_proxy_common(
        ...
        if base_asset_amount_received > locked_token_amount_available {
            ...
            let unlocked_tokens = DcdtTokenPayment::new(asset_token_id.clone(), 0, unlocked_amount);

            // burn base asset, as we only need to send the locked tokens
            self.send()
                .dcdt_local_burn(&asset_token_id, 0, &attributes.locked_tokens.amount);
            ...
            let asset_token_id = received_token_refs
                ...
                .clone();
            self.send()
                .dcdt_local_burn(&asset_token_id, 0, &locked_tokens_out.amount);
    ...
    fn increase_proxy_pair_token_energy_endpoint(&self, lock_epochs: Epoch) -> DcdtTokenPayment {
        self.require_wrapped_lp_token_id_not_empty();
