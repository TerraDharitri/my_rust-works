...
deployStakeFarmContract() {
    ...
    ADDRESS=$(drtpy data parse --file="deploy-stake-farm-internal.interaction.json" --expression="data['contractAddress']")
...
addAddressToWhitelist() {
    whitelist_address="0x$(drtpy wallet bech32 --decode $2)"
...
claimRewards() {
    ...
    drtpy --verbose contract call $OWNER --recall-nonce \
        --pem=${WALLET_PEM} \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
        --gas-limit=500000000 \
