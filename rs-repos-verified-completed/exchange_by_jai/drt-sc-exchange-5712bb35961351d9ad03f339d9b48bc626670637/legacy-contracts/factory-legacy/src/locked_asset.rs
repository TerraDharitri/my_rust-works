...
pub struct LockedTokenEx<M: ManagedTypeApi> {
    pub token_amount: DcdtTokenPayment<M>,
...
pub trait LockedAssetModule: attr_ex_helper::AttrExHelper {
    fn create_and_send_locked_assets(
        ...
        let last_created_nonce = self.send().dcdt_nft_create_compact(
            &token_id,
            &(amount + additional_amount_to_create),
            attributes,
        );
        self.send()
            .direct_dcdt(address, &token_id, last_created_nonce, amount);
    ...
    fn add_quantity_and_send_locked_assets(
        ...
        let token_id = self.locked_asset_token_id().get();
        self.send().dcdt_local_mint(&token_id, sft_nonce, amount);

        self.send()
            .direct_dcdt(address, &token_id, sft_nonce, amount);
    }
    ...
    fn mint_and_send_assets(&self, dest: &ManagedAddress, amount: &BigUint) {
        if amount > &0 {
            ...
            self.send().dcdt_local_mint(&asset_token_id, 0, amount);
            self.send().direct_dcdt(dest, &asset_token_id, 0, amount);
        }
    }
