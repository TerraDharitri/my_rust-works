...
pub struct LockedUnlockedTokenRefPair<'a, M: ManagedTypeApi> {
    pub locked_token_ref: &'a DcdtTokenPayment<M>,
    pub unlocked_token_ref: &'a DcdtTokenPayment<M>,
}

pub struct BaseAssetOtherTokenRefPair<'a, M: ManagedTypeApi> {
    pub base_asset_token_ref: &'a DcdtTokenPayment<M>,
    pub other_token_ref: &'a DcdtTokenPayment<M>,
}
...
pub trait ProxyCommonModule: energy_query::EnergyQueryModule {
    fn require_exactly_one_locked<'a>(
        &self,
        first_payment: &'a DcdtTokenPayment,
        second_payment: &'a DcdtTokenPayment,
    ...
    fn burn_if_base_asset(&self, tokens: &DcdtTokenPayment) {
        let asset_token_id = self.get_base_token_id();
        if tokens.token_identifier == asset_token_id {
            self.send()
                .dcdt_local_burn(&tokens.token_identifier, 0, &tokens.amount);
        }
    }
