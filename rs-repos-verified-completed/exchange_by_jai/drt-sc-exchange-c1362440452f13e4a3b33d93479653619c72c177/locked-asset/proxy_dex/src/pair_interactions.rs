...
pub struct AddLiquidityResultWrapper<M: ManagedTypeApi> {
    ...
    pub first_token_leftover: DcdtTokenPayment<M>,
    pub second_token_leftover: DcdtTokenPayment<M>,
}

pub struct RemoveLiqudityResultWrapper<M: ManagedTypeApi> {
    pub first_token_received: DcdtTokenPayment<M>,
    pub second_token_received: DcdtTokenPayment<M>,
}
...
pub trait PairInteractionsModule {
    fn call_add_liquidity(
        ...
        let first_payment =
            DcdtTokenPayment::new(first_token_id, 0, first_token_amount_desired.clone());
        let second_payment =
            DcdtTokenPayment::new(second_token_id, 0, second_token_amount_desired.clone());
        ...
        let first_token_leftover = DcdtTokenPayment::new(
            first_tokens_used.token_identifier,
            0,
            first_token_leftover_amount,
        );
        let second_token_leftover = DcdtTokenPayment::new(
