...
pub(crate) async fn enter_farm(
    ...
    let result_token = dex_interact
        ...
        .payment::<DcdtTokenPayment<StaticApi>>(lp_token.into())
...
pub(crate) async fn claim_rewards(
    ...
    let payments = payment
        .iter()
        .map(DcdtTokenPayment::from)
        .collect::<ManagedVec<StaticApi, DcdtTokenPayment<StaticApi>>>();
...
pub(crate) async fn exit_farm(
    ...
    let result_token = dex_interact
        ...
        .payment::<DcdtTokenPayment<StaticApi>>(payment.into())
        .returns(ReturnsResult)
        .prepare_async()
        .run()
        .await;
