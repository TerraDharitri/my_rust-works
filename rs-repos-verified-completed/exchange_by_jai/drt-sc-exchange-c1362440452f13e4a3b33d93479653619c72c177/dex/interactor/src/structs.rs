use dharitri_sc_scenario::{
    api::StaticApi,
    imports::{
        Address, BigInt, BigUint, DcdtTokenPayment, ManagedAddress, ManagedTypeApi, ManagedVec,
...
impl<M: ManagedTypeApi>
    From<MultiValue3<DcdtTokenPayment<M>, DcdtTokenPayment<M>, DcdtTokenPayment<M>>>
    for InteractorAddLiquidityResultType
{
    fn from(
        value: MultiValue3<DcdtTokenPayment<M>, DcdtTokenPayment<M>, DcdtTokenPayment<M>>,
    ) -> Self {
...
impl<M: ManagedTypeApi> From<DcdtTokenPayment<M>> for InteractorToken {
...
impl<M: ManagedTypeApi> From<InteractorToken> for DcdtTokenPayment<M> {
...
impl AddArgs {
    pub fn as_payment_vec(
    ...
    ) -> ManagedVec<StaticApi, DcdtTokenPayment<StaticApi>> {
        ...
        let mut payments = ManagedVec::from_single_item(DcdtTokenPayment::new(
...
impl SwapArgs {
    pub fn as_payment(&self, dex_interact: &mut DexInteract) -> DcdtTokenPayment<StaticApi> {
        let first_token_id = dex_interact.state.first_token_id().as_bytes();
        DcdtTokenPayment::new(
            TokenIdentifier::from(first_token_id),
            0,
