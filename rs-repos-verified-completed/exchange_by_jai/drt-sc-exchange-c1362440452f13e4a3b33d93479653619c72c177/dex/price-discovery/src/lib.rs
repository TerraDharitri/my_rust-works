    ...
    + dharitri_sc_modules::default_issue_callbacks::DefaultIssueCallbacksModule
{
    ...
    fn withdraw(&self) -> RewaOrDcdtTokenPayment<Self::Api> {
        ...
        let (refund_token_id, balance_mapper) = match payment_nonce {
            LAUNCHED_TOKEN_REDEEM_NONCE => (
                RewaOrDcdtTokenIdentifier::dcdt(self.launched_token_id().get()),
        ...
        self.tx()
            .to(&caller)
            .rewa_or_single_dcdt(&refund_token_id, 0, &withdraw_amount)
            .transfer();
    ...
    fn compute_bought_tokens(
        ...
        RewaOrDcdtTokenPayment::new(token_id, 0, reward_amount)
    }
