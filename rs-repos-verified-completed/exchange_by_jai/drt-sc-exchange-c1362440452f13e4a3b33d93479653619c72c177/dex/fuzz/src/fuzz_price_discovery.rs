...
pub mod fuzz_price_discovery_test {
        ...
        PriceDiscObjBuilder: 'static + Copy + Fn() -> price_discovery::ContractObj<DebugApi>,
    {
        ...
        let caller_token_before =
            fuzzer_data
                .blockchain_wrapper
                .get_dcdt_balance(&caller.address, payment_token_id, 0);

        let redeem_token_before = fuzzer_data.blockchain_wrapper.get_dcdt_balance(
            &caller.address,
            redeem_token_id,
            redeem_token_nonce,
        );

        let sc_launched_token_before = fuzzer_data.blockchain_wrapper.get_dcdt_balance(
            price_disc.pd_wrapper.address_ref(),
            DISC_LAUNCHED_TOKEN_ID,
            0,
        );

        let sc_accepted_token_before = fuzzer_data.blockchain_wrapper.get_dcdt_balance(
            price_disc.pd_wrapper.address_ref(),
        ...
        let tx_result = fuzzer_data.blockchain_wrapper.execute_dcdt_transfer(
            &caller.address,
        ...
        let redeem_token_after = fuzzer_data.blockchain_wrapper.get_dcdt_balance(
            &caller.address,
            redeem_token_id,
            redeem_token_nonce,
        );

        let sc_launched_token_after = fuzzer_data.blockchain_wrapper.get_dcdt_balance(
            price_disc.pd_wrapper.address_ref(),
            DISC_LAUNCHED_TOKEN_ID,
            0,
        );

        let sc_accepted_token_after = fuzzer_data.blockchain_wrapper.get_dcdt_balance(
            price_disc.pd_wrapper.address_ref(),
            DISC_ACCEPTED_TOKEN_ID,
            0,
        );
