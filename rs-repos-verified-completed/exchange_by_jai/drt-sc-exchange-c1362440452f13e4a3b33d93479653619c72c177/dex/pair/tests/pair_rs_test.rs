...
fn test_swap_fixed_input() {
    ...
    pair_setup.swap_fixed_input(WREWA_TOKEN_ID, 1_000, MOA_TOKEN_ID, 900, 996);
}
...
fn test_swap_fixed_output() {
    ...
    pair_setup.swap_fixed_output(WREWA_TOKEN_ID, 1_000, MOA_TOKEN_ID, 900, 96);
}
...
fn test_perfect_swap_fixed_output() {
    ...
    pair_setup.swap_fixed_output(WREWA_TOKEN_ID, 1_000, MOA_TOKEN_ID, 996, 0);
...
fn add_liquidity_through_simple_lock_proxy() {
    ...
    pair_setup.b_mock.check_nft_balance(
        ...
        &rust_biguint!(2_000_000),
        Some(&LockedTokenAttributes::<DebugApi> {
            original_token_id: managed_token_id_wrapped!(MOA_TOKEN_ID),
            original_token_nonce: 0,
            unlock_epoch: 15,
        }),
    );
    ...
    // add liquidity through simple-lock SC - one locked (WREWA) token, one unlocked (MOA)
    let transfers = vec![
        TxTokenTransfer {
        ...
        TxTokenTransfer {
            token_identifier: MOA_TOKEN_ID.to_vec(),
            nonce: 0,
            value: rust_biguint!(500_000),
        },
    ];
...
fn fees_collector_pair_test() {
    ...
    pair_setup
        ...
        .execute_tx(
            ...
            |sc| {
                ...
                tokens.push(managed_token_id!(MOA_TOKEN_ID));
    ...
    pair_setup.swap_fixed_input(WREWA_TOKEN_ID, 100_000, MOA_TOKEN_ID, 900, 90_669);
