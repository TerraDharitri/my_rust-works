...
pub trait PairMock {
    ...
    fn init(
        ...
        self.first_token_id().set(
            first_token_id
                .into_option()
                .as_ref()
                .unwrap_or(&TokenIdentifier::from_dcdt_bytes(DEFAULT_FIRST_TOKEN_ID)),
        );
        self.second_token_id().set(
            second_token_id
                .into_option()
                .as_ref()
                .unwrap_or(&TokenIdentifier::from_dcdt_bytes(DEFAULT_SECOND_TOKEN_ID)),
        );
        self.lp_token_id().set(
            lp_token_id
                .into_option()
                .as_ref()
                .unwrap_or(&TokenIdentifier::from_dcdt_bytes(DEFAULT_LP_TOKEN_ID)),
        );
    ...
    fn add_initial_liquidity(&self) -> AddLiquidityResultType<Self::Api> {
        ...
        if !self.skip_minting_lp_tokens().get() {
            self.send().dcdt_local_mint(&lp_token_id, 0, &liquidity);
        }
        ...
        (
            ...
            DcdtTokenPayment::new(expected_first_token_id, 0, BigUint::zero()),
            DcdtTokenPayment::new(expected_second_token_id, 0, BigUint::zero()),
        )
            .into()
    }
    ...
    fn get_tokens_for_given_position_with_safe_price(
        ...
        (
            DcdtTokenPayment::new(self.first_token_id().get(), 0, liquidity.clone() / 2u64),
            DcdtTokenPayment::new(self.second_token_id().get(), 0, liquidity / 2u64),
        )
            .into()
    }
