...
pub struct EnterFarmResult<M: ManagedTypeApi> {
    pub farming_token: DcdtTokenPayment<M>,
    pub farm_token: DcdtTokenPayment<M>,
    pub rewards: DcdtTokenPayment<M>,
}

pub type EnterFarmProxyResultType<M> = MultiValue2<DcdtTokenPayment<M>, DcdtTokenPayment<M>>;
pub type ExitFarmProxyResultType<M> = MultiValue2<DcdtTokenPayment<M>, DcdtTokenPayment<M>>;
pub type ClaimRewardsFarmProxyResultType<M> = MultiValue2<DcdtTokenPayment<M>, DcdtTokenPayment<M>>;
    ...
    + sc_whitelist_module::SCWhitelistModule
{
    ...
    fn enter_farm_proxy_endpoint(
        ...
        let new_wrapped_farm_token = if token_merge_requested {
            ...
            self.send().dcdt_local_burn_multi(&payments);
    ...
    fn enter_farm_locked_token(
        ...
        user: ManagedAddress,
        farm_address: ManagedAddress,
        locked_token: DcdtTokenPayment,
    ) -> EnterFarmResult<Self::Api> {
        let asset_token_id = self.get_base_token_id();
        self.send()
            .dcdt_local_mint(&asset_token_id, 0, &locked_token.amount);

        let minted_asset_tokens = DcdtTokenPayment::new(asset_token_id, 0, locked_token.amount);
    ...
    fn enter_farm_wrapped_lp(
        ...
        let farming_token = DcdtTokenPayment::new(
    ...
    fn increase_proxy_farm_token_energy_endpoint(&self, lock_epochs: Epoch) -> DcdtTokenPayment {
        self.require_wrapped_farm_token_id_not_empty();
        self.require_wrapped_lp_token_id_not_empty();
