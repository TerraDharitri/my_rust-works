    ...
    setup
        ...
        .execute_dcdt_transfer(
            ...
            |sc| {
                ...
                let wrewa_payment = results.other_token_payment;
                assert_eq!(
                    wrewa_payment.token_identifier,
                    managed_token_id!(WREWA_TOKEN_ID)
                );
                assert_eq!(wrewa_payment.amount, dual_yield_token_amount / 2);
    ...
    setup
        ...
        .execute_dcdt_transfer(
            ...
            |sc| {
                ...
                assert_eq!(wrewa_payment.amount, 1_001_000_000 / 2);
...
fn claim_for_others_positive_test() {
    ...
    setup
        .b_mock
        .check_dcdt_balance(&temp_user, RIDE_TOKEN_ID, &rust_biguint!(0));
    setup.b_mock.check_dcdt_balance(
        &user_address,
        RIDE_TOKEN_ID,
...
fn total_farm_position_after_claim_and_exit_metastaking_test() {
    ...
    setup
        .b_mock
        .set_dcdt_balance(&temp_user, WREWA_TOKEN_ID, &rust_biguint!(300_000_000u64));
