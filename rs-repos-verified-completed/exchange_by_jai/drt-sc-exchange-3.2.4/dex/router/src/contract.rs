    ...
    + enable_swap_by_user::EnableSwapByUserModule
{
    ...
    fn create_pair_endpoint(
        ...
        require!(
            first_token_id.is_valid_dcdt_identifier(),
            "First Token ID is not a valid dcdt token ID"
        );
        require!(
            second_token_id.is_valid_dcdt_identifier(),
            "Second Token ID is not a valid dcdt token ID"
        );
    ...
    fn lp_token_issue_callback(
        ...
        let (token_id, returned_tokens) = self.call_value().rewa_or_single_fungible_dcdt();
        match result {
