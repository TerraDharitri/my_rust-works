...
use crate::{
    ...
    wrewa_swap_setup::{WrewaSwapSetup, WREWA_TOKEN_ID},
};
...
> where
    ...
    WrewaSwapBuilder: 'static + Copy + Fn() -> dharitri_sc_wrewa_swap::ContractObj<DebugApi>,
    ComposableTasksBuilder: 'static + Copy + Fn() -> composable_tasks::ContractObj<DebugApi>,
{
    ...
    pub wrewa_swap_setup: WrewaSwapSetup<WrewaSwapBuilder>,
...
impl<PairBuilder, RouterBuilder, WrewaSwapBuilder, ComposableTasksBuilder>
    ComposableTasksSetup<PairBuilder, RouterBuilder, WrewaSwapBuilder, ComposableTasksBuilder>
where
    PairBuilder: 'static + Copy + Fn() -> pair::ContractObj<DebugApi>,
    RouterBuilder: 'static + Copy + Fn() -> router::ContractObj<DebugApi>,
    WrewaSwapBuilder: 'static + Copy + Fn() -> dharitri_sc_wrewa_swap::ContractObj<DebugApi>,
    ComposableTasksBuilder: 'static + Copy + Fn() -> composable_tasks::ContractObj<DebugApi>,
{
    pub fn new(
        ...
        wrewa_swap_builder: WrewaSwapBuilder,
        ...
        let wrewa_swap_setup = WrewaSwapSetup::new(b_mock.clone(), wrewa_swap_builder, &owner);
        ...
        b_mock
            ...
            .execute_tx(&owner, &ct_wrapper, &rust_biguint!(0), |sc| {
                ...
                let wrewa_swap_addr: &Address = wrewa_swap_setup.wrewa_swap_wrapper.address_ref();
                sc.set_wrap_rewa_address(managed_address!(wrewa_swap_addr));
