...
use dharitri_sc::{
    ...
    types::{Address, DcdtTokenPayment, MultiValueEncoded},
};
    ...
    FarmStakingProxyObjBuilder: 'static + Copy + Fn() -> farm_staking_proxy::ContractObj<DebugApi>,
{
    ...
    pub fn call_pair_add_liquidity(
        ...
        self.b_mock
            .execute_dcdt_multi_transfer(caller, &self.pair_wrapper, &payments, |sc| {
        ...
        self.b_mock
            .execute_dcdt_transfer(
                ...
                |sc| {
                    ...
                    let new_expected_payment = DcdtTokenPayment::new(
    ...
    pub fn setup_new_user(&mut self, token_id: &[u8], token_amount: u64) -> Address {
        ...
        self.b_mock
            .set_dcdt_balance(&new_user, token_id, &rust_biguint!(token_amount));
        new_user
    }
}
