    ...
    + dharitri_sc_modules::default_issue_callbacks::DefaultIssueCallbacksModule
{
    ...
    fn enter_metastaking_endpoint(&self, metastaking_address: ManagedAddress) -> DcdtTokenPayment {
        ...
        if metastaking_state.dual_yield_amount > 0 {
            let empty_payment =
                DcdtTokenPayment::new(payment.token_identifier.clone(), 0u64, BigUint::zero());
        ...
        let current_metastaking_position = DcdtTokenPayment::new(
            dual_yield_token_id,
    ...
    fn unstake_metastaking(&self) -> PaymentsVec<Self::Api> {
        ...
        let division_safety_constant =
            self.get_division_safety_constant(farm_staking_address.clone());
        let _full_dual_yield_position = DcdtTokenPayment::new(
        ...
        let unstake_dual_yield_tokens = DcdtTokenPayment::new(
            dual_yield_token_id.clone(),
            metastaking_state.dual_yield_token_nonce,
        ...
        let update_dual_yield_tokens = DcdtTokenPayment::new(
            dual_yield_token_id,
            metastaking_state.dual_yield_token_nonce,
        ...
        if user_lp_farm_reward > 0 {
            let user_lp_farm_reward_payment = DcdtTokenPayment::new(
        ...
        if user_staking_reward > 0 {
            let user_staking_reward_payment = DcdtTokenPayment::new(
    ...
    fn unbond_metastaking_endpoint(&self) -> DcdtTokenPayment<Self::Api> {
        let payment = self.call_value().single_dcdt();
        require!(
        ...
        let unbond_payment = DcdtTokenPayment::new(
        ...
        let caller = self.blockchain().get_caller();
        self.send()
            .direct_non_zero_dcdt_payment(&caller, &unbond_payment);
    ...
    fn claim_and_compute_user_metastaking_rewards(
        ...
        let user_staking_reward_payment =
            DcdtTokenPayment::new(staking_token_id, 0u64, user_staking_reward);
