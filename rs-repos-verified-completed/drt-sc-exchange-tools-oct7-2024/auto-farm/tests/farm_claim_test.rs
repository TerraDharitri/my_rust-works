...
use dharitri_sc::types::{BigInt, DcdtTokenPayment, ManagedVec, MultiValueEncoded};
...
fn claim_rewards_through_auto_farm() {
    ...
    farm_setup
        .b_mock
        .borrow_mut()
        .execute_dcdt_multi_transfer(&first_user, &auto_farm_wrapper, &farm_tokens, |sc| {
    ...
    farm_setup
        ...
        .execute_tx(&proxy_address, &auto_farm_wrapper, &rust_zero, |sc| {
            ...
            expected_user_farm_tokens.push(DcdtTokenPayment::new(
            ...
            let expected_user_rewards = MergedRewardsWrapper::<DebugApi> {
                opt_locked_tokens: Some(DcdtTokenPayment::new(
...
fn withdraw_specific_farm_tokens_test() {
    ...
    farm_setup
        ...
        .execute_tx(&first_user, &auto_farm_wrapper, &rust_zero, |sc| {
            ...
            tokens_to_withdraw.push(DcdtTokenPayment::new(
                managed_token_id!(FARM_TOKEN_ID[FIRST_FARM_INDEX]),
                1,
