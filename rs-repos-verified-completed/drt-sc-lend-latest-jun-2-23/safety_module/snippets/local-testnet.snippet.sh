...
DCDT_TICKER=0x5745474c442d623265666236 # WREWA-b2efb6
DCDT_NAME=0x5745474c44 # WREWA

DCDT_BUSD_TICKER=0x425553442d343533386638 #BUSD-4538f8
...
sendREWAToContract(){
    drtpy tx new --receiver ${ADDRESS} --value=1000000000000000000 --pem=${BOB} --recall-nonce --gas-limit=1000000000 --proxy=${PROXY} --chain=${CHAIN_ID} --function=issue --outfile="issueWREWA.json" --arguments 0X5745474c44 0X5745474c44 100000 10 --send
}

issueWREWA(){
    drtpy contract call ${ESTD_ISSUE_ADDRESS} --value=5000000000000000000 --pem=${BOB} --recall-nonce --gas-limit=1000000000 --proxy=${PROXY} --chain=${CHAIN_ID} --function=issue --outfile="issueWREWA.json" --arguments 0X5745474c44 0X5745474c44 100000 0x02 --send
}

issueBUSD(){
    drtpy contract call ${ESTD_ISSUE_ADDRESS} --value=5000000000000000000 --pem=${BUSD_POOL} --recall-nonce --gas-limit=1000000000 --proxy=${PROXY} --chain=${CHAIN_ID} --function=issue --outfile="issueBUSD.json" --arguments 0X42555344 0X42555344 100000 --send
}
...
fundFromPool(){
    drtpy contract call ${ADDRESS} --pem=${BUSD_POOL}  --recall-nonce --gas-limit=1000000000 --proxy=${PROXY} --chain=${CHAIN_ID} --function=DCDTTransfer --arguments ${DCDT_BUSD_TICKER} 50015 0x66756e6446726f6d506f6f6c --send
}
