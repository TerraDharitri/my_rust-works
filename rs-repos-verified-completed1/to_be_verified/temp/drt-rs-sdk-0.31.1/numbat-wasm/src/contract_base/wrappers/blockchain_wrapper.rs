...
use crate::{
    ...
    storage::{self},
    types::{
        BigUint, DcdtLocalRoleFlags, DcdtTokenData, ManagedAddress, ManagedByteArray, ManagedType,
        TokenIdentifier,
    },
};
    ...
    A: BlockchainApi + ManagedTypeApi + ErrorApi,
{
    ...
    pub fn get_current_dcdt_nft_nonce(
        ...
        A::blockchain_api_impl()
            .get_current_dcdt_nft_nonce(address.get_raw_handle(), token_id.get_raw_handle())
    }
    ...
    pub fn get_dcdt_token_data(
        ...
        A::blockchain_api_impl().get_dcdt_token_data::<A>(address, token_id, nonce)
    }

    #[inline]
    pub fn get_dcdt_local_roles(&self, token_id: &TokenIdentifier<A>) -> DcdtLocalRoleFlags {
        A::blockchain_api_impl().get_dcdt_local_roles(token_id.get_raw_handle())
    }
}
    ...
    A: BlockchainApi + StorageReadApi + ManagedTypeApi + ErrorApi,
{
    ...
    pub fn get_cumulated_validator_rewards(&self) -> BigUint<A> {
        ...
        A::storage_read_api_impl()
            .storage_load_big_uint_raw(storage::protected_keys::NUMBAT_REWARD_KEY, result_handle);
        BigUint::from_raw_handle(result_handle)
    }
}
